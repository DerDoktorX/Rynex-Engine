...
namespace Rynex {
    ...
    public:
        ...
        {
            uint32_t curentSize = VertexQuadeTransformBatche<T, N, MaxTex, VerticiesQuadMax>::template Batching<T>::Size();
            if (curentSize < m_BeginUpdate)
            {
                m_BeginUpdate = curentSize;
            }
            VertexQuadeTransformBatche<T, N, MaxTex, VerticiesQuadMax>::template Batching<T>::Emplace_Back_Push(data);
        }
        ...
        void SubmitDarw()
        {
            ...
            VertexQuadeTransformBatche<T, N, MaxTex, VerticiesQuadMax>::template Batching<T>::MoveElementPtr();
        }
    private:
