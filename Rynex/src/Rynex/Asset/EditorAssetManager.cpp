...
#pragma region AssetRegistry
    ...
    bool AssetRegistry::IsAssetInternal(AssetHandle handle) const
    {
        return IsAssetInRegistry(handle) && GetMetadataConst(handle).GetIntern();
    }

    void AssetRegistry::CreateAsset(const FileSystem::Path& path, AssetHandle handle, AssetMetadata metadata,
                                    bool findDirectOnDisc)
    {
        if (!IsAssetInRegistry(path) && IsAssetPath(path))
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    assetFileDirectory.m_FolderPath = parent;
                    RY_ASSET_WARN("This Should Not Happened!");
                }
            }
    ...
    {
        while (UUID::Zero() == asset->m_Handle && IsAssetInRegistry(asset->m_Handle))
        {
    ...
    bool AssetRegistry::IsCurrentAssetState(const FileSystem::Path& showPath) const
    {
        return m_Changes || showPath != m_CurrentPath;
    }

    ContentBrowserItems AssetRegistry::GetCurrentAssetInformation(const FileSystem::Path& showPath)
    {
...
#pragma region EditorAssetManagerThreade
    ...
    void EditorAssetManagerThread::OnAttach()
    {
        ...
        const FileSystem::Path basePath(baseDir);
...
#pragma region IsAsset
    ...
    {
        std::lock_guard<std::mutex> scope(m_WorkingThreadMutex);
        std::function<void(FileSystem::Path, bool*)> lambder = [this
            ](FileSystem::Path pathLambder, bool* resultLambder) -> void
        {
...
#pragma region GetData
    ...
    {
        std::lock_guard<std::mutex> scope(m_WorkingThreadMutex);
        std::function<void(AssetHandle, AssetMetadata*)> lambder = [this
            ](AssetHandle handleLambder, AssetMetadata* metadataLambder) -> void
        {
    ...
    void EditorAssetManagerThread::GetMetadataAsync(const FileSystem::Path& path, AssetMetadata* meatdata)
    {
        ...
        std::function<void(FileSystem::Path handleLambder, AssetMetadata* metadataLambder)> lambder = [this
            ](FileSystem::Path handleLambder, AssetMetadata* metadataLambder) -> void
        {
            *metadataLambder = GetMetadata(handleLambder);
        };
    ...
    void EditorAssetManagerThread::GetAssetHandleAsync(const FileSystem::Path& path, AssetHandle* handle)
    {
        ...
        std::function<void(FileSystem::Path handleLambder, AssetHandle* metadataLambder)> lambder = [this
            ](FileSystem::Path pathLambder, AssetHandle* handleLambder) -> void
        {
        ...
        if (IsAssetHandleValid(handle))
        {
            ...
            else if (!IsAssetInteral(handle))
            {
                ...
                if (metadataC.GetIntern())
                {
                    RY_CORE_ASSERT(
                        false,
                        "Error on: 'EditorAssetManager::GetAsset' Asset is known In Regestry as Interale-Asset But got not Loadede! This cinde off error is a def Error");
                    return nullptr;
                }
            ...
            {
                RY_CORE_ASSERT(
                    false, "Error on: 'EditorAssetManager::GetAsset' I Dont't know some isuse kan go to hear!");
                return nullptr;
            }
            return asset;
        }
...
#pragma region LocaleAssetsDataMage
    ...
    {
        while (IsAssetHandleValid(asset->m_Handle))
            asset->m_Handle = AssetHandle();
        AssetHandle handle = asset->m_Handle;
...
#pragma region DiscFile
    ...
    void EditorAssetManagerThread::ScanDirectory(const FileSystem::Path& current)
    {
        if (current.IsEmpty())
            return;

        const std::filesystem::path& currentPath = current.GetPath();
        for (const std::filesystem::directory_entry& p : std::filesystem::directory_iterator(currentPath))
        {
        ...
        SetPathUpdateContentBrowser(current);
    }
    ...
    void EditorAssetManagerThread::ReLodeAsset(AssetHandle handle)
    {
        ...
        if (IsAssetLoaded(handle))
        {
            ...
            {
                int about = 0;
                SetAssetMetadataLockState(handle, std::bind(&EditorAssetManagerThread::SetAssetMetadataStateUpdating,
                                                            this, &about, std::placeholders::_1));
                if (about == 1)
                {
            ...
            if (result)
                SetAssetMetadataLockState(handle, std::bind(&EditorAssetManagerThread::SetAssetMetadataStateReady, this,
                                                            std::placeholders::_1));
            else
                SetAssetMetadataLockState(handle, std::bind(&EditorAssetManagerThread::SetAssetMetadataStateError, this,
                                                            std::placeholders::_1));
    ...
    {
        SetAssetMetadataLockState(handle, std::bind(&EditorAssetManagerThread::SetAssetMetadataStateLoading, this,
                                                    std::placeholders::_1));
            ...
            if (IsAssetLoaded(handle))
            {
                ...
                default:
                    if (loadtime != std::chrono::steady_clock::time_point::min())
                    {
                        ...
                        int64_t differenc = std::chrono::duration_cast<std::chrono::milliseconds>(curent - loadtime).
                            count();
        ...
        if (IsDirectoryInRegistry(showPath))
        {
            ContentBrowserItemesThreade itemes;
            {
                m_DirectoryRegistry.Read<bool>(
                    [this, &itemes, &showPath](
                    const std::map<FileSystem::Path, AssetFileDirectoryThreade>& assetBrowserDataThread) -> bool
                    {
    ...
    {
        m_WorkingThreadMutex.lock();
        m_WorkingThread.emplace_back(std::async(std::launch::async, [this](const FileSystem::Path pathLembder)
        {
            std::string name = pathLembder.GetNamePathString();
            RY_ASSET_WARN("Execute Added Event -> Path: '{}' | Scan Dir: '{}'", name, pathLembder);
            ScanDirectory(pathLembder);
        }, path));
        m_WorkingThreadMutex.unlock();
    }
    ...
    bool EditorAssetManagerThread::CheckAssetFileExist(AssetHandle handle)
    {
        ...
        if (!IsAssetPathExtensionValid(metaData.m_Path))
        {
            SetAssetMetadataLockState(handle, std::bind(&EditorAssetManagerThread::SetAssetMetadataStateError, this,
                                                        std::placeholders::_1));;
        }
        ...
        {
            SetAssetMetadataLockState(handle, std::bind(&EditorAssetManagerThread::SetAssetMetadataStateLostConnection,
    ...
    void EditorAssetManagerThread::CreateAssetDirectory(const FileSystem::Path& path)
    {
        ...
        const FileSystem::Path parentPath = path.GetParent();
        std::filesystem::path folderRelativePath = parentPath.GetRelativePath();
        ...
        std::string name = path.GetNamePathString();
        AssetFileDirectoryThreade assetFileDirectory;
        assetFileDirectory.FolderName = name;
        assetFileDirectory.FolderPath = path;

        m_DirectoryRegistry.Add(path, assetFileDirectory);
    ...
    void EditorAssetManagerThread::AddDirectory(const FileSystem::Path& folder, const FileSystem::Path& parentFolder,
                                                std::map<FileSystem::Path, AssetFileDirectoryThreade>& folderMap)
    {
        ...
        else if (IsAssetHandleValid(path))
        {
            ...
            else
            {
                ...
                if (IsAssetLoaded(handle))
                {
                    ...
                    {
                        RY_CORE_FATAL_IF(
                            AssetState::Ready == metaData.m_State || AssetState::Loading == metaData.m_State,
                            "Interesting Stat is not Ready or Current Loading, but is stored in LoadedAssets Map! \nMaby Youst Asyc Action");
                    }, handle);
                }
                else
                {
                    SetAssetMetadataLockState(handle, std::bind(
                                                  &EditorAssetManagerThread::SetAssetMetadataStateNotLoaded, this,
                                                  std::placeholders::_1));
                }
    ...
    void EditorAssetManagerThread::CreateNewFileAsset(const FileSystem::Path& path)
    {
...

        {
            ...
            std::lock_guard<std::mutex> lockPath(m_CurrentPathMutex);
            const FileSystem::Path parent = path.GetParent();
            if (m_CurrentPath == parent)
                m_FileChanges = true;
        ...
        m_HandleRegistry.Add(handle, metaData);
        m_PathRegistry.Add(metaData.m_Path, handle);
        m_PathRegistry.Add(metaData.m_Path, handle);

        if (m_PathRegistry.IsFound(metaData.m_Path))
            m_PathRegistry.Add(metaData.m_Path, handle);
    }
