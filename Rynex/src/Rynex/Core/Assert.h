...
#define RY_CORE_NOT_IMPL() RY_CORE_FATAL("Function Not Implemted {} {}", std::filesystem::path(__FILE__).string(), __LINE__); RY_DEBUG_BREAK()
	...
	#define RY_INTERALE_ASSERT_WITH_MSG(type, check, msg) RY_INTERALE_ASSERT_IMPL(type, check, "Assertion Faild: {0}", msg)
	#define RY_INTERALE_ASSERT_WITH_MSG_ARGS(type, check, msg, msgArg1, ...) RY_INTERALE_ASSERT_IMPL(type, check, "Assertion Faild: " msg,  msgArg1, __VA_ARGS__)
	...
	#define RY_INTERALE_ASSERT_GET_MACRO_NAME(arg1, arg2, msgArg, marco, ...) marco
	#define RY_INTERALE_ASSERT_GET_MACRO(...) RY_EXPAND_MOAKRO(  RY_INTERALE_ASSERT_GET_MACRO_NAME(__VA_ARGS__,RY_INTERALE_ASSERT_WITH_MSG_ARGS, RY_INTERALE_ASSERT_WITH_MSG,  RY_INTERALE_ASSERT_NO_MSG)  )
