...
namespace Rynex::UI {
    ...
    namespace Utils {
        constexpr const char* s_VECTOR_LABEL_ARRAY[4] = {
            "X",
            "Y",
            "Z",
            "W"
        ...
        constexpr const char* s_VECTOR_DRAG_LABEL_ARRAY[4] = {
            "##X",
            "##Y",
            "##Z",
            "##W"
        ...
        static bool AttributeControllerResetType(const char* label, const char* labelDrag, T& value, const glm::vec3 color, const ImVec2& buttenSize, T resetValue, float hoverColorDifference, float activeColorDifference, const char* resetTip, const char* charValue, DragFunc dragFunc)
        {
        ...
        static ImVec2 ControllerStart(const std::string& label, const ShaderDataType shaderType, const uint32_t componentCount, const float collumWidth)
        {
        ...
        static bool AttributeControllerReset(const char* label, const char* dragLabel, float& value, const glm::vec3 color, const ImVec2& buttenSize, const float resetValue, const float hoverColorDifference = -0.075f, const float activeColorDifference = 0.1f)
        {
            return AttributeControllerResetType(label, dragLabel,value, color, buttenSize, resetValue, hoverColorDifference, activeColorDifference, "Reset Value %.2f", "%.2f", &ImGui::DragFloat);
        }

        static bool AttributeControllerReset(const char* label, const char* dragLabel, int& value, const glm::vec3 color, const ImVec2& buttenSize, const int resetValue, const float hoverColorDifference = -0.075f, const float activeColorDifference = 0.1f)
        {
            return AttributeControllerResetType(label, dragLabel, value, color, buttenSize, resetValue, hoverColorDifference, activeColorDifference, "Reset Value %i", "%i", &ImGui::DragInt);
        }

        template<typename T, int componentCount>
        static bool DrawVectorControllerReset(const std::string& label, glm::vec<componentCount, T>& values, glm::vec<componentCount, T> resetValue, const float collumWidth, const ShaderDataType shaderType)
        {
            bool changes = false;
            ImGuiScopeID scopeID(label);
            const ImVec2 buttenSize = ControllerStart(label, shaderType, componentCount, collumWidth);

            for (int i = 0; i < componentCount; i++)
            {
                changes = Utils::AttributeControllerReset(s_VECTOR_LABEL_ARRAY[i], s_VECTOR_DRAG_LABEL_ARRAY[i], values[i], s_VECTOR_COLOR_ARRAY[i], buttenSize, resetValue[i]) || changes;
            }
        ...
        template<typename T, int componentCount>
        static bool DrawVectorController(const std::string& label, glm::vec<componentCount, T>& values,const float collumWidth, const ShaderDataType shaderType)
        {
            bool changes = false;
            ImGuiScopeID scopeID(label);
            const ImVec2 buttenSize = ControllerStart(label, shaderType, componentCount, collumWidth);
            ...
            for (int i = 0; i < componentCount; i++)
            {
                changes = Utils::AttributeControllerReset(s_VECTOR_LABEL_ARRAY[i], s_VECTOR_DRAG_LABEL_ARRAY[i], values[i], s_VECTOR_COLOR_ARRAY[i], buttenSize, 0.0f) || changes;
            }
    ...
    bool DrawIVec4ControllerR(const std::string& label, glm::ivec4& values, const glm::ivec4 resetValue, const float collumWidth)
	{
        constexpr ShaderDataType shaderType = ShaderDataType::Int4;
		return Utils::DrawVectorControllerReset<int, 4>(label, values, resetValue, collumWidth, shaderType);
	}

	bool DrawVec4Controller(const std::string& label, glm::vec4& values, const float resetValue, const float collumWidth)
	{
#if 0
		bool changes = false;
		ImGuiIO& io = ImGui::GetIO();
        ...
        return Utils::DrawVectorControllerReset<float, 4>(label, values, resetValueVEc, collumWidth, shaderType);
#endif

	}

	bool DrawVec4ControllerR(const std::string& label, glm::vec4& values, const glm::vec4 resetValue, const float collumWidth)
	{
#if 0
		bool changes = false;
		ImGuiIO& io = ImGui::GetIO();
...
#else
        constexpr ShaderDataType shaderType = ShaderDataType::Float4;
        return Utils::DrawVectorControllerReset<float, 4>(label, values, resetValue, collumWidth, shaderType);
#endif

	}

	bool DrawVec3ControllerR(const std::string& label, glm::vec3& values, const glm::vec3 resetValue, const float collumWidth)
	{
#if 0
		bool changes = false;
		ImGuiIO& io = ImGui::GetIO();
        ...
        return Utils::DrawVectorControllerReset<float, 3>(label, values, resetValue, collumWidth, shaderType);
#endif
	}

	bool DrawVec3Controller(const std::string& label, glm::vec3& values, float resetValue, const float collumWidth)
	{
#if 0
		bool changes = false;
		ImGuiIO& io = ImGui::GetIO();
...
#else
        constexpr ShaderDataType shaderType = ShaderDataType::Float3;
        return Utils::DrawVectorController<float, 3>(label, values, collumWidth, shaderType);
#endif

	}

	bool DrawVec2Controller(const std::string & label, glm::vec2& values, float resetValue, const float collumWidth)
	{
#if 0
		bool changes = false;
		ImGuiIO& io = ImGui::GetIO();
...
#else
        constexpr ShaderDataType shaderType = ShaderDataType::Float3;
        return Utils::DrawVectorController<float, 2>(label, values, collumWidth, shaderType);
#endif

	}

	bool DrawFloatController(const std::string& label, float* values, float resetValue, const float min, const float max, const float collumWidth)
	{
#if 0
		bool changes = false;
		ImGuiIO& io = ImGui::GetIO();
    ...
    const ImVec2 buttenSize = Utils::ControllerStart(label, shaderType, componentCount, collumWidth);
    float& v = *values;
    bool change =  Utils::AttributeControllerReset(Utils::s_VECTOR_LABEL_ARRAY[0], Utils::s_VECTOR_DRAG_LABEL_ARRAY[0], v, Utils::s_VECTOR_COLOR_ARRAY[0], buttenSize, resetValue);
